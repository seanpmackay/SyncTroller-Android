...
public class MainActivity extends BridgeActivity {
  ...
  public void onCreate(Bundle savedInstanceState) {
        ...
        webView.evaluateJavascript(
          ...
          (String value) -> {
            if (!"true".equals(value)) {
              ...
              // "close this Activity" -- SyncStatusService is a foreground
              // service (see its own comment: kept alive "for as long as a
              // Sync Box is paired", independent of whether this Activity
              // is open), so finish() alone left it and its notification
              // running and the process alive, with Force Stop the only
              // way to actually kill it. Stopping it here first removes
              // the one thing keeping the process around once the
              // Activity is gone; it starts right back up next launch
              // (services.js re-shows it on boot whenever a box is still
              // paired) so nothing about pairing state is lost by this.
              stopService(new Intent(MainActivity.this, SyncStatusService.class));
              finish();
            }
          }
        );
    ...
    // it the status notification's foreground service still runs fine, it
    // just won't display anything. Asked once up front rather than only at
